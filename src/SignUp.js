    ...
    <div className="login-container">
      <div className="post-card-login">
        ...
        <div className="login-textbox">
          ...
          <input type="text" id="text-username" placeholder="Last Name" value={LastName} onChange={(event)=>setLastName(event.target.value)} />

          <input type="text" id="text-username" placeholder="Username" value={inputValueUserName} onChange={(event)=>setInputValueUserName(event.target.value)}/>

          <input type="password" id="text-username" placeholder="Password" value={inputValueUserPassword} onChange={(event)=>setInputValuePassword(event.target.value)} />

          <button onClick={handleSubmit} id="btn-signup">Sign Up</button>
        </div>
      </div>
      
      <div className='post-card-login-footer'>
          <p>Have an account? <a href='http://localhost:3000/login'><span>Log In</span></a></p>
        </div>
    </div>
  );
}
